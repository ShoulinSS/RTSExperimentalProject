...
@group(2) @binding(112) var<uniform> map_scale: f32;

@group(2) @binding(113) var fog_of_war: texture_2d<f32>;
@group(2) @binding(114) var fog_sampler: sampler;
...
fn fragment(
    ...
    let fog_uv = vec2<f32>(
        (pbr.world_position.x / map_scale) + 0.5,
        (pbr.world_position.z / map_scale) + 0.5
    );

    let fog_value = textureSample(fog_of_war, fog_sampler, fog_uv).r;
    let visibility_multiplier = mix(0.5, 1.0, fog_value);

    out.color = vec4<f32>(
        out.color.rgb * visibility_multiplier,
        out.color.a
    );
